...
namespace Zen.Data
{
    public class NHibernateDao : IGenericDao
    {
        ...
        /// Threads servicing client requests obtain ISessions from the factory. 
        /// </remarks>
        ...
        /// </remarks>
        /// <param name="key">
        /// AssociationPath = property name for the object on the other end 
        /// of the association
        /// </param>
        ...
        public IEnumerable<T> Fetch<T>(Query query) where T : class, new()
        {
            return Fetch<T>(query, 0, 0);
        }

        public IEnumerable<T> Fetch<T>(Query query, int pageIndex, int pageSize) where T : class, new()
        {
            IQuery qry = null;
            switch (query.QueryType)
            {
                ...
                case QueryTypes.Sql:
                    ...
                    if (query.Aliases == null || query.Aliases.Count < 1)
        ...
        public IEnumerable<T> FetchAll<T>() where T : class, new()
        {
            return FetchAll<T>(0, 0);
        }

        public IEnumerable<T> FetchAll<T>(int pageIndex, int pageSize) where T : class, new()
        {
        ...
        public T FetchByCompositeId<T>(IDictionary<string, object> compositeId) where T : class, new()
        {
            ...
            if (list.Count() > 1) throw new DataAccessException(string.Format("Type[{0}] CompositeId has multiple[{1}] results!", typeof(T), list.Count()));
            // we should have 1 and only 1
            foreach (var item in list) return item;
            throw new DataAccessException(string.Format("Type[{0}] CompositeId has impossible[{1}] results!", typeof(T), list.Count()));
        }

        public IEnumerable<T> FetchByExample<T>(T entity, string[] excludePropertyList) where T : class, new()
        {
            return FetchByExample(entity, excludePropertyList, 0, 0);
        }

        public IEnumerable<T> FetchByExample<T>(T entity, string[] excludePropertyList, int pageIndex, int pageSize) where T : class, new()
        {
            var criteria = CreateCriteria<T>(null, pageIndex, pageSize);
        ...
        public IEnumerable GetNamedQueryList(string queryName, IDictionary<string, object> parms)
        {
            var namedQuery = _session.GetNamedQuery(queryName);
            foreach (var parm in parms)
                namedQuery.SetParameter(parm.Key, parm.Value);
